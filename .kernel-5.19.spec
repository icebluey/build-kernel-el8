...
# Packages that need to be present before kernel is installed
...
BuildRequires: findutils flex gawk gcc git gzip hmaccalc hostname
BuildRequires: kmod m4 make net-tools openssl openssl-devel patch
BuildRequires: perl-Carp perl-devel perl-generators perl-interpreter
BuildRequires: python3-devel redhat-rpm-config rsync sh-utils tar xz
%if %{with_doc}
...
%ifarch x86_64
%if %{with_default}
%{__cp} -f config-%{version}-%{_target_cpu} .config
...
# compression of the kernel module files.
#
%define __spec_install_post \
    %{__arch_install_post} \
    %{__os_install_post} \
    if [ "%{zipmodules}" -eq "1" ]; then \
        %{_bindir}/find $RPM_BUILD_ROOT/lib/modules/ -type f -name '*.ko' | \
            %{_bindir}/xargs --no-run-if-empty -I "{}" %{__chmod} 644 "{}";\
        %{_bindir}/sleep 10; \
